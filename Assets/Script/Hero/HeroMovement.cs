...
public class HeroMovement : MonoBehaviour
{
    ...
    if(GetComponent<HeroStats>()._elementalType == Elements.ElementalAttribute.Earth)
    {
        if(!GetComponent<HeroActions>().onWall && Input.GetButton("Grab"))
        {
            wallGrab = true;
        }
        if(!GetComponent<HeroActions>().onWall && Input.GetButtonUp("Grab"))
        {
            wallGrab = false;
        }
        if(wallGrab)
