...
public class SmashCamera : MonoBehaviour
{
    ...
    private void FindAveragePosition()
    {
        ...
        for (int i = 0; i < PlayerManager._playersList.Count; i++)
        {
            ...
            if (PlayerManager._playersList[i] != null)
            {
                if (!PlayerManager._playersList[i].gameObject.activeSelf)
                    continue;
            }
            ...
            if (PlayerManager._playersList[i] != null)
            {
                averagePos += PlayerManager._playersList[i].transform.position;
                numTargets++;
            }
        }
    ...
    private float FindRequiredSize()
    {
        ...
        for (int i = 0; i < PlayerManager._playersList.Count; i++)
        {
            ...
            if (PlayerManager._playersList[i] != null)
            {
                targetLocalPos = transform.InverseTransformPoint(PlayerManager._playersList[i].transform.position);
            }
